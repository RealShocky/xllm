...
stopwords = ( "of", "now", "have", "so", "since", "but", "and", 
              ...
              "specified","using", "somewhat", "cannot", "because", "taken",
              "over", "expressed", "expressing", "he", "provide",
              "polya~random", 
            )
...
# unless otherwise specified, word and word2 consists of 1, 2, 3, or 4 tokens
...
def save_tables(): 
    ...
    list = { "dictionary" : dictionary,
             ...
             "hash_see" : hash_see,
             "compressed_word2_hash": compressed_word2_hash,
           }
    ...
    for key in word2_pairs:
        ...
        if dictionary[word2A] > 1 and dictionary[word2B] > 1:
            if word2_pairs[key] > 0:
                file.write(str(key) + "\t" + str(word2_pairs[key]) + "\n")
    file.close()

    file = open("xllm6_arr_url.txt", "w")
...
print("Word pairs (up to 4 tokens):", len(word2_pairs)) 
...
while query != "":  
    ...
    for query in queries:
        ...
        stemmed = llm6.stem_data(token_list, stopwords, dictionary, mode = 'Internal')   
